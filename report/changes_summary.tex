...
Two model variants are trained with lookback lengths
$L \in \{1,8\}$ and learn
\begin{equation}
...
Training minimizes a one-step loss plus pushforward closed-loop rollout loss:
\begin{equation}
\mathcal{L}(\theta)
...
\right),
\end{equation}
where $K=5$ pushforward steps and $\lambda=0.7$ in the implementation.
...
\begin{align}
h^{(1)} &= \mathrm{LSTM}_{64}^{(1)}(\mathbf{X}_k),\\
h^{(2)} &= \mathrm{LSTM}_{64}^{(2)}(h^{(1)}),\\
\hat a_{k+L} &= W h^{(2)} + b,
\end{align}
...
Improved validation loss from: 3.9862821131464445e-05  to: 3.793928490267717e-05
Validation R2: 0.9999605
Test loss: 1.588151e-05
Test R2: 0.99996436
Test rollout RMSE: 0.004790624603629112
Performing inference on testing data
Model restored successfully!
Making predictions on testing data
One-step input RMSE per mode: [1.2709553 0.4815025 1.2855033]
Multistep input RMSE per mode: [1.2374445 1.242674 0.9698531]
[onestep] Deployment RMSE (mean over modes): 1.0126537
[multistep] Deployment RMSE (mean over modes): 1.1499906
...
\subsection{One-step vs Multistep Comparison}
The repository now trains and evaluates two forecasting variants under the same data split:
\begin{itemize}
  \item \textbf{One-step input model}: $L=1$, autoregressive one-step deployment.
  \item \textbf{Multistep input model}: $L=8$, autoregressive one-step deployment.
\end{itemize}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Model} & \textbf{Mode 0 RMSE} & \textbf{Mode 1 RMSE} & \textbf{Mode 2 RMSE} & \textbf{Mean RMSE} \\
\midrule
One-step input ($L=1$) & 1.2709553 & 0.4815025 & 1.2855033 & 1.0126537 \\
Multistep input ($L=8$) & 1.2374445 & 1.2426740 & 0.9698531 & 1.1499906 \\
\bottomrule
\end{tabular}
\caption{Autoregressive deployment RMSE comparison from \texttt{build/run\_unbuffered.log}.}
\end{table}
...
\begin{itemize}
  ...
  \item Checkpoints: \texttt{build/checkpoints/my\_checkpoint\_onestep.pt}, \texttt{build/checkpoints/my\_checkpoint\_multistep.pt}
